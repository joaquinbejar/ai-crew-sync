...
- **`list_conversations` before `create_conversation`**, and always after a reconnect. It lists the threads **this window** can read: the ones it was invited to or joined, plus project threads its agent's project access covers (archived ones only with `include_archived: true`). An invited thread is listed too: join it rather than creating a duplicate. **An empty list does not mean no thread exists.** Visibility and membership differ: a private thread you have no seat in is never listed, and membership is per window, so a thread a sibling window (or your session before a restart) sits in is not yours. Listing grants nothing either: to take part you still need an invitation you accept with `join_conversation`, so ask a member to invite this window's `agent/session`, or have the window that holds the seat hand it over with `transfer_membership`.
